...
describe('更新资源按 update 前的行锁状态分派缓存失效', () => {
  ...
  // 反向: 快照 status=0, 并发隐藏后该行已是 1, 编辑未拦截时行保持 1
  it('快照为公开但 update 前已被隐藏, 不再多余失效', async () => {
    resourceFindUniqueMock.mockResolvedValue(buildSnapshot())
    transactionQueryRawMock.mockResolvedValue([
      { status: 1, section: 'galgame' }
    ])
    transactionResourceUpdateMock.mockResolvedValue(buildUpdated({ status: 1 }))

    // name/note/model 均未变更 → 不送审, 不触发拦截
    await updatePatchResource(buildInput({ name: 'Old name' }), 9, 3)
