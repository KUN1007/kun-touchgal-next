...
import { POST } from '~/app/api/user/setting/allow-private-message/route'

const createRequest = (body: unknown) =>
  new Request('http://localhost/api/user/setting/allow-private-message', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
...
describe('POST /api/user/setting/allow-private-message', () => {
  ...
  it('未登录时返回错误字符串且不写库', async () => {
    ...
    const response = await POST(createRequest({ allowPrivateMessage: true }))
    const body = await response.json()
  ...
  it('按请求中的期望值绝对赋值而非翻转', async () => {
    const response = await POST(createRequest({ allowPrivateMessage: false }))
    const body = await response.json()

    expect(userUpdateManyMock).toHaveBeenCalledWith({
      where: { id: 7 },
      data: { allow_private_message: false }
    })
