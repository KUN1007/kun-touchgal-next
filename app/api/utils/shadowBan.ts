...
// WHERE fragment for patch_comment / patch_rating (status: 0 normal, 1 shadow banned)
export const getShadowBanWhere = (viewer: KunViewer | null) => {
  if (viewer && viewer.role >= 3) {
    return {}
  }
  if (viewer && viewer.uid > 0) {
    return { OR: [{ status: 0 }, { user_id: viewer.uid }] }
  }
  return { status: 0 }
}

// WHERE fragment for patch_resource; status=2 (pending approval) stays hidden
...
export const getShadowBanSql = (viewer: KunViewer | null) =>
  viewer && viewer.role >= 3
    ? Prisma.sql`TRUE`
    : Prisma.sql`(status = 0 OR user_id = ${viewer?.uid ?? 0})`
